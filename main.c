//KYALO KELVIN MUINDI
//P15/37561/2016
//ELECTION ALGORITHM(BULLY ALGORITHM) IMPLEMENTATION
#include<stdio.h>
#include<conio.h>
#include<stdlib.h>
...
void bully() {
   ...
   for (i = 0; i < n; i++) {
      ...
      printf("\nIs process with id [%d] Active ?[0/1] :", i + 1);
      scanf("%d", &a);
   ...
   printf("\n\nProcess with the id [%d] is the co-ordinator", max);
      ...
      while (1) {
         printf("\n\nEnter wether the process with id [%d] is active or not? [0/1]: ",
               p2->no);
         scanf("%d", &p2->active);
         p2 = p2->next;
         if (p2 == head)
      ...
      printf("\n\nElection message is sent to process");
      ...
      printf("\n\nProcess with id [%d] says I Won", max);
